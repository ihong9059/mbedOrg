...
rs485Status_t* rs485::return485Status(){
	return &m_status;
}

uint8_t* rs485::returnTestFrame(){
	return mtData;
}
...
//			m_status.count=0;
	...
	else if(m_status.count>tFLENGTH){
			m_status.flag=false;
			m_status.count=0;
			m_status.start=false;
			m_testDone=false;
...
//	Serial sendDown(p9, p11);	//FOR Test
...
void rs485::sendByUp(){
	wait(0.2);
	Serial sendUp(p9, p11);		//For Server status data Test
