...
void test::testRfReceive(){
	UttecUtil myUtil;
	pMyRf->changeGroup(DeRfTestChannel);
	pMyRf->sendRf(mp_rfFrame);
	testTimeout.attach(&testTimeoutProc, DeRfTestTimeout);
	testFlag = true;	
	uint8_t ucResult = 0;
	while(testFlag){
		myUtil.setWdtReload();
	...
	if(ucResult>DeRfTestCount) printf("Ok Rf Communication:%d\r\n",ucResult);
	else printf("Rf Communication Error:%d\r\n", ucResult);
	pMyRf->changeGroup(mp_rfFrame->MyAddr.GroupAddr);
	pMyRf->sendRf(mp_rfFrame);
	
			/*
	//for nrfTester, Rp.cpp: Rf::initRf dbm_m20, Flash.cpp G:100
			static	uint16_t uiRssiCount = 0;
			if(!(uiRssiCount++%100)){
				Uart.putc('$');
				pMyFrame->Cmd.Command = edDummy;
				pMyFrame->Trans.DstGroupAddr = uiRssiCount;
